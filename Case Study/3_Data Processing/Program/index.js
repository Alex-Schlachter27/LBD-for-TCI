...
const getWalls = async () => {
    ...
    const endpoint = "http://localhost:3030/TCI-CaseStudy-Revit/query";
    ...
    const q = `
        ...
        ?s  tci:pciType product:Wall ;
            ...
            FILTER (?Level = "N00" || ?Level = "N1" || ?Level = "N1Koblingszone" )
        }
        ORDER BY ?ID`;
...
const getTCIs = async () => {
    ...
    const endpoint = "http://localhost:3030/TCI-CaseStudy-TCI/query";
...
// (4) Loop through all walls and take wall length as a variable
    ...
    // console.log(wall)
        ...
        var adjElArray = wall.adjacentElements
        var adjElArray = wall.adjacentElements
        // console.log(adjElArray.length)
        // console.log(adjElArray)
        if (adjElArray.length === 1) {
          var adjacentWall1 = adjElArray[0]
        //   console.log(adjacentWall1)
          ...
          var adjacentWalls = [adjacentWall1]
        }

        else if ((adjElArray.length === 2)) {
          var adjacentWall1 = adjElArray[0]
          var adjacentWall2 = adjElArray[1]
        ...
        var InsideCorner = CorTCIs.find(obj => obj.TCIinst === 'http://test/tci/DefaultInsideCorner270x20');
        var OutsideCorner = CorTCIs.find(obj => obj.TCIinst === 'http://test/tci/DefaultOutsideCorner270x55');
        ...
        // Because of stacked elements, add two instances for each corner
        if (Cornercount == 90) {
            wallLength -= (0.175+0.20);
            ...
            // count double for stacked formwork elements
            count[InsideCorner.TCIinst]=0.5*2;
            count[OutsideCorner.TCIinst]=0.5*2;
        } else if (Cornercount == 180) {
            wallLength -= 2*(0.175+0.20);
            InsideCorner1 = {'TCIinst': InsideCorner.TCIinst + '_0', 'length': Iv};
            InsideCorner2 = {'TCIinst': InsideCorner.TCIinst + '_1', 'length': Iv};
            InsideCorner3 = {'TCIinst': InsideCorner.TCIinst + '_2', 'length': Iv};
            InsideCorner4 = {'TCIinst': InsideCorner.TCIinst + '_3', 'length': Iv};
            OutsideCorner1 = {'TCIinst': OutsideCorner.TCIinst + '_0', 'length': Ov};
            OutsideCorner2 = {'TCIinst': OutsideCorner.TCIinst + '_1', 'length': Ov};
            OutsideCorner3 = {'TCIinst': OutsideCorner.TCIinst + '_2', 'length': Ov};
            OutsideCorner4 = {'TCIinst': OutsideCorner.TCIinst + '_3', 'length': Ov};
            wall.TCIsIn.push(InsideCorner1, InsideCorner2, InsideCorner3, InsideCorner4);
            wall.TCIsOut.push(OutsideCorner1, OutsideCorner2, OutsideCorner3, OutsideCorner4);
            // count double for stacked formwork elements
            count[InsideCorner.TCIinst]=0.5*2*2;
            count[OutsideCorner.TCIinst]=0.5*2*2;
        }
        // console.log(wall)
        ...
        while (wallLength > minLengthOfTCI) {
            ...
            // Count the amount of each TCI type and return as key-value pair --> count double for stacked formwork elements
            count[TCIinstvalue]=(TCIinstcount+2)*2;
            // count.TCIclass = TCIinstvalue; // Not working as TCIclass gets overwritten
            // count.Count = TCIinstcount+1;
            // console.log(count)

            // Because of stacked formwork elements, create two instances for each selected TCI (_0, _1)
            // cloning the variable selected TCI in to not have the numbers add up after each wall iteration
            s1 = clone(selectedTCI)
            v1 = clone(TCIinstvalue)
            selectedTCI1 = s1
            TCIinstvalue1 = v1+= '_'+TCIinstcount;
            selectedTCI1.TCIinst = TCIinstvalue1;

            s2 = clone(selectedTCI)
            v2 = clone(TCIinstvalue)
            selectedTCI2 = s2
            TCIinstcount+=1
            TCIinstvalue2 = v2+= '_'+TCIinstcount;
            selectedTCI2.TCIinst = TCIinstvalue2;

            // TCIinstvalue+= '_'+TCIinstcount;
            // selectedTCI.TCIinst = TCIinstvalue;
            // console.log(TCIinstvalue)
            // console.log(selectedTCI.TCIinst)
            // console.log(selectedTCI1, selectedTCI2)
            
            // cloning the variable selected TCI in to not have the numbers add up after each wall iteration
            wall.TCIsIn.push(selectedTCI1, selectedTCI2); 
            // console.log(wall.TCIsIn);
            wall.TCIsOut.push(selectedTCI1, selectedTCI2);
            // console.log(wall.TCIsOut);
        ...
        // Calculation for both sides of the wall and two elements on top of each other
            const Couplercount = (TCIsum)*2+bigPanelscount*2+smallPanelscount*1;
...
//         else if ( wall.Connections.length === 2) {
//             const insturl1 = wall.Connections[0].wallinst;
...
//         const endpoint = "http://localhost:3030/TCI-CaseStudy/update";
